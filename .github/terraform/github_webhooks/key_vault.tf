...
resource "azurerm_key_vault" "SSPHP" {
  ...
  access_policy {
    tenant_id = azurerm_linux_function_app.SSPHP_github_webhooks.identity[0].tenant_id
    object_id = azurerm_linux_function_app.SSPHP_github_webhooks.identity[0].principal_id
