resource "azurerm_service_plan" "SSPHP_github_webhooks" {
  name                = "SSPHP-GitHub-Webhooks-${local.postfix}"
...
resource "azurerm_linux_function_app" "SSPHP_github_webhooks" {
  name                       = "SSPHP-GitHub-Webhooks-${local.postfix}"
  resource_group_name        = azurerm_resource_group.tfstate.name
  location                   = azurerm_resource_group.tfstate.location
  tags                       = var.tags
  ...
  service_plan_id            = azurerm_service_plan.SSPHP_github_webhooks.id
  enabled                    = true
  ...
  zip_deploy_file = data.archive_file.github_webhooks.output_path
